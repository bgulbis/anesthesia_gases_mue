...
```{r, fig.cap="Distribution of cost by type of gas used"}
data_gas %>%
    # left_join(data_surgeries, by = c("millennium.id", "surg.start.datetime")) %>%
...
```{r, fig.cap="Anesthesia gas flow rates over the duration of surgery"}
data_gas_realtime %>%
    ...
    scale_y_continuous("Gas Flow Rate", breaks = seq(0, 20, 2)) +
    scale_color_brewer("Gas Used", palette = "Dark2") +
    theme_bg() +
    theme(legend.title = element_text(color = "grey35"))
```
...
data_gas_realtime %>%
    ...
    geom_smooth() +
    scale_x_continuous("Surgery Duration (hours)", breaks = seq(0, 20, 2)) +
    scale_y_continuous("Fresh Flow Rate", breaks = seq(0, 20, 2)) +
    scale_color_brewer("Gas Used", palette = "Dark2") +
    theme_bg() +
    theme(legend.title = element_text(color = "grey35"))
```
